function [img0, img1, kp_homo_initial_matched, state, T_cw, ...
    descriptors_current_matched, reprojection_errors, costs] = ...
    ...
    %% Extraction of matching keypoints
    ...
    kp_homo_query_fl = kp_homo_current_matched([2 1 3], :);
    ...
    kp_homo_current_matched = kp_homo_current_matched(:, inlier_mask);
    ...
    if (debug_verbose)
        figure(5); 
        subplot(2,1,1);
        showMatchedFeatures(initial_frame, current_frame, flipud(kp_initial_matched)', ...
            flipud(kp_current_matched)', 'montage');       
        subplot(2,1,2);
